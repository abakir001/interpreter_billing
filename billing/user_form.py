...
class CaseInsensitiveUsernameFieldCreationForm(UserCreationForm):
    ...
    are saved. This is to disallow the existence of phone number usernames which differ only in
    case.
    """
...
CaseInsensitivePhoneUserCreationForm = CaseInsensitiveUsernameFieldCreationForm
...
class FriendlyPasswordResetForm(OldPasswordResetForm):
    ...
    error_messages['unknown'] = _("This phone number doesn't have an "
                                  "associated user account. Are you "
                                  "sure you've registered?")

    def clean_phone(self):
        """Return an error message if the phone number being reset is unknown.
        """

        phone = self.cleaned_data['phone']
        results = list(self.get_users(phone))
